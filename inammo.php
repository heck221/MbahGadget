...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		if(isset($_COOKIE["ghosthaxorapi"])){
			if(!@file_exists("alfacgiapi/".$_COOKIE["ghosthaxorapi"].".alfa")){
				$recreate = true;
		...
		$cgi_url = __ALFA_DATA_COK__."/alfacgiapi/".$lang.$cgi_ext;
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["alfacgiapi_mode"])&&!isset($_COOKIE["ghosthaxorapi"])){
